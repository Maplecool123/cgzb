...
class grid_eval_cpb_xml
{
   ...
   function inicializa_vars()
   {
      ...
      $this->Arquivo     .= ".xml";
      $this->Tit_doc      = "grid_eval_cpb.xml";
   ...
   function grava_arquivo()
   {
      ...
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_eval_cpb']['xml_name']))
      {
          $this->Arquivo = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_eval_cpb']['xml_name'];
          $this->Tit_doc = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_eval_cpb']['xml_name'];
          unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_eval_cpb']['xml_name']);
      }
      if (!$this->Grava_view)
      {
      ...
      while (!$rs->EOF)
      {
         $this->xml_registro = "<grid_eval_cpb";
         $this->mc = $rs->fields[0] ;  
...
$_SESSION['scriptcase']['grid_eval_cpb']['contr_erro'] = 'off'; 
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->mc))
         {
             $this->mc = sc_convert_encoding($this->mc, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " mc =\"" . $this->trata_dados($this->mc) . "\"";
   }
   //----- sfzg
   function NM_export_sfzg()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->sfzg))
         {
             $this->sfzg = sc_convert_encoding($this->sfzg, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " sfzg =\"" . $this->trata_dados($this->sfzg) . "\"";
   }
   //----- isdefault
   function NM_export_isdefault()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->isdefault))
         {
             $this->isdefault = sc_convert_encoding($this->isdefault, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " isdefault =\"" . $this->trata_dados($this->isdefault) . "\"";
   }
   //----- tjsj
   ...
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->cpbview))
         {
             $this->cpbview = sc_convert_encoding($this->cpbview, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " cpbview =\"" . $this->trata_dados($this->cpbview) . "\"";
   }
   //----- intro
   function NM_export_intro()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->intro))
         {
             $this->intro = sc_convert_encoding($this->intro, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " intro =\"" . $this->trata_dados($this->intro) . "\"";
   }
   //----- tjr
   function NM_export_tjr()
   {
         if ($_SESSION['scriptcase']['charset'] == "UTF-8" && !NM_is_utf8($this->tjr))
         {
             $this->tjr = sc_convert_encoding($this->tjr, "UTF-8", $_SESSION['scriptcase']['charset']);
         }
         $this->xml_registro .= " tjr =\"" . $this->trata_dados($this->tjr) . "\"";
   }
   ...
   function monta_html()
   {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_eval_cpb']['xml_file']);
      if (is_file($this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo))
      {
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_eval_cpb']['xml_file'] = $this->Ini->root . $this->Ini->path_imag_temp . "/" . $this->Arquivo;
      }
...
<HEAD>
 <TITLE><?php echo $this->Ini->Nm_lang['lang_othr_grid_titl'] ?> - <?php echo $this->Ini->Nm_lang['lang_tbl_eval_cpb'] ?> :: XML</TITLE>
